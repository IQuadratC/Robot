...
extern int* referenceData;

extern int* newData;

extern std::vector<MapData> MainMap;

int threadFehler[360];
...
int calculateFehler(int w)
{
    ...
    for (int i = 0; i < 360; i++)
    {
        ...
        if (angle < 0)
            angle += 359;
        if (referenceData[angle] != -1) {
            fehler += abs(referenceData[i] - newData[angle]);
        }
    }
...
int calculateAngle()
{
    ...
    for (auto i : threadFehler) fehlermap[i.second] = i.first;


    int min_error = std::numeric_limits<int>::max();
    int angle = 0;
    for (int i = 0; i < 360; i++) if (min_error > fehlermap[i])
    {
        min_error = fehlermap[i];
        angle = i;
    }
